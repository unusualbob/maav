 ...
 * @param {String} [paramName] Used to alter the message of ObjectParameterError to your preference based on the user's input field name
 * @returns {ValidatorObject}
 */
function Validator(sourceObj, paramName = 'sourceObj') {
  if (sourceObj != null && typeof sourceObj !== 'object') {
    throw new ObjectParameterError(sourceObj, paramName, 'Validator');
  }
  if (!this.$__schema) {
  ...
  if (sourceObj) {
    ...
    if (sourceObj instanceof mongoose.Document) {
      this.$isNew = sourceObj.$isNew;
    }
  }
